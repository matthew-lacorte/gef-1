...
def calculate_full_potential_derivative(
    ...
    """Compute δU/δφ for the full GEF Lagrangian.

    This function implements the Euler-Lagrange derivative of the potential energy `U`,
    which acts as the "force" driving the relaxation of the scalar field `φ`. Each
    term corresponds to a core physical principle of the GEF framework.

    Potentials (U):
    ----------------
    U_iso     = -½ μ² φ² + ¼ λ φ⁴
        The standard "Mexican Hat" potential. The λφ⁴ term ensures a stable
        vacuum, while the -μ²φ² term drives spontaneous symmetry breaking, allowing
        for non-trivial vacuum states (φ ≠ 0).

    U_P       = -P (1-φ²)
        An environmental pressure term. It models the external pressure from the
        Plenum, favoring the true vacuum state (φ² = 1) and penalizing the
        false vacuum (φ = 0).

    U_aniso   = ½ g² (1-φ²)² (∂_w φ)²
        The Anisotropic Stabilizer, implementing the κ-flow. It penalizes gradients
        exclusively in the w-dimension (time), breaking the SO(4) symmetry and
        allowing for stable, persistent 4D structures. The (1-φ²)² factor
        confines this effect to the vacuum, not the particle's core.

    U_hook    = ½ h² (1-φ²) [ (∂_x φ)² + (∂_y φ)² ]
        The Hook Coupling. This term models the short-range, high-strength
        interaction (Strong Force) via direct topological interlocking.
        Its components are physically motivated:
        - `h²`: The Hook Coupling constant, representing the "shear modulus"
          or tensile strength of the Φ-field substrate.
        - `(∂_x φ)² + (∂_y φ)²`: The geometric "twist tensor." Measures the
          degree of topological entanglement between solitons in the spatial dimensions.
        - `(1-φ²)`: The Core Confinement Factor. This is a crucial switch that
          ensures the interaction is only active where φ deviates from its vacuum
          value (φ² ≠ 1), i.e., *inside the core of the particles*. This
          mathematically enforces the observed short-range nature of the
          nuclear forces.
