...
This version includes a 'quantize-gravity' mode to simulate the co-evolution
of local geometry and physical laws, searching for stable, self-consistent
...
# ==============================================================================
# 2. NEW CORE ITERATOR FOR QUANTIZED GRAVITY
# ==============================================================================
@njit(inline='always')
def gef_iter_quantized(z: complex, c: complex, cos_th: float, sin_th: float,
    ...
    """
    # Feedback loop: local gravity determines the physical "law" (anisotropy)
    # The law "wants" to be genesis_target, and the local k_flow "pulls" it.
...
# ==============================================================================
# 3. RENDER ENGINES (with NEW Quantized Gravity Renderer)
# ==============================================================================
@njit(parallel=True)
def _render_fractal_quantized(width: int, height: int, x_min: float, x_max: float,
        ...
        for py in prange(height):
            ...
            for px in range(width):
                ...
                if img[py, px] == 0:
                    ...
                    # Proxy for mass density: low magnitude = high density
                    # A small |z| creates a deep time well (low κ value)
    ...
    # The final image is not the escape time, but the state of the "law" itself.
    # We visualize the final anisotropy field, which is a function of the k_flow_grid.
...
# ... (_render_fractal and _render_orbit remain unchanged for other modes) ...
@njit(parallel=True)
...
# ... (Plotting and Analysis functions remain the same) ...
def plot_and_save_frame(img_data: np.ndarray,
    ...
    Parameters:
        img_data: The image data to plot
        frame_path: Path to save the image
        title: Title for the plot
        cmap: Colormap name
        dpi: DPI for the saved image
        figsize: Figure size (width, height) in inches
        extent: Plot extent [xmin, xmax, ymin, ymax]
        log_norm: Whether to use logarithmic normalization
        norm_type: Normalization type: "linear" (default), "relative", "percentile"
    """
    fig, ax = plt.subplots(figsize=figsize, facecolor="black")
    
    # Choose normalization based on the type and data
    if log_norm and img_data.max() > 0:
        # Logarithmic normalization
        norm = colors.LogNorm(vmin=1, vmax=img_data.max())
    elif norm_type == "relative":
        # Relative normalization - scales to the actual range of data
        # This helps when changing max_iterations
...
# ==============================================================================
# 5. CONFIGURATION AND ORCHESTRATION (Updated for new mode)
# ==============================================================================
def load_and_validate_config(config_path: Path) -> Dict:
    # ... (this function remains the same, but we add 'quantize-gravity' to valid modes) ...
    ...
    # Add new mode to the list of valid modes
    if cfg['mode'] not in ['mandelbrot', 'julia', 'orbit', 'quantize-gravity']:
    ...
    if cfg['mode'] != 'quantize-gravity' and 'sweep_parameters' not in cfg:
...
def run_simulation(config: Dict):
    # ... (Setup is mostly the same, dispatch to new render function) ...
    mode = config['mode']
    ...
    # --- Quantize Gravity Mode ---
    if mode == 'quantize-gravity':
        p_qg = config['quantize_gravity_params']
        print(f"Running Quantize Gravity simulation with target={p_qg['genesis_target']}, "
              f"strength={p_qg['genesis_strength']}")
        
        # In this mode, theta is fixed, we don't sweep it.
        th_pi = p_qg.get('fixed_angle_pi', 0.0)
        th_rad = th_pi * np.pi
        cos_th, sin_th = math.cos(th_rad), math.sin(th_rad)
        
        img_data = _render_fractal_quantized(
            ...
            genesis_target=p_qg['genesis_target'],
            genesis_strength=p_qg['genesis_strength'],
            k_damping=p_qg['k_damping']
        )
        
        title = (f"Genesis State | target={p_qg['genesis_target']:.3f}, "
                 f"strength={p_qg['genesis_strength']:.3f}")
        
        plot_and_save_frame(
            img_data, output_dir / "genesis_state.png", title,
            config['plotting']['cmap'], config['plotting']['dpi'],
            tuple(config['plotting']['animation_figsize']), config['fractal_window'],
            log_norm=False, norm_type=config['plotting'].get('normalization', 'relative')
        )
        print("\nSimulation complete.")
        return

    # --- Standard Modes (Mandelbrot, Julia, Orbit) ---
    ...
    # ... (rest of the standard simulation loop) ...
    ...
    # Get normalization type from config, default to "linear"
...
def main():
    ...
    # Let's point to a new config for this special mode
